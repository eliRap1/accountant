...
export default function Logo({ className = "", showWordmark = true }: Props) {
  // useId ensures each Logo instance gets a unique gradient id, preventing
  // duplicate SVG id attributes when Logo appears in both Navbar and Footer.
  const uid = useId();
  const gradientId = `acg-${uid.replace(/:/g, "")}`;
