 ...
 * `{ip}`, `{ua}` as markers because string concatenation inside a multiline
 * array is fragile. Unknown markers are left in place so a missing var
 * shows up loudly in QA rather than silently disappearing.
 */
export function fillText(
  text: string,
  vars: { url?: string; ip?: string | null; ua?: string | null },
): string {
  return text
